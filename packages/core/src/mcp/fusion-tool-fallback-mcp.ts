    ...
    switch (request.method) {
      case "initialize":
        return jsonRpcResult(id, {
          ...
          protocolVersion,
          serverInfo: {
            name: "omr-fusion-tool-fallback",
            title: "OMR Fusion Tool Fallback",
            version: "1.0.0"
          }
        });
      case "ping":
  ...
  return {
    content: [{
      text: tool?.unavailableMessage ||
        ...
        "OMR registered a fallback definition because the real MCP server did not provide the tool during discovery. " +
